...
def Continuous_Paradigm(Integration_Continuous, num_Spectrometer_Samples, num_DAC_Samples, num_power_samples, num_background_Spec):
    '''
    A function used to read data for the Continuous Paradigm
    '''
    #First take the samples of the readings of the Power Meter -- Only runs when connected for test run
    power_process = Process(target=power_read_process, args=(num_power_samples,))
    spectrometer_init_process = Process(target=spectrometer_init_process, args=(cont_integration_time,))
    spectrometer_test_process = Process(target=spectrometer_read_process, args=(num_background_spec,))
    spectrometer_read_process = Process(target=spectrometer_read_process, args=(num_spectrometer_samples,))
    timer_process = Process(target=timer_multi_process, args=(0.1,))
    daq_process = Process(target=DAQ_Read_Process, args=(num_DAC_samples,))
    shutter_process = Process(target=DAQ1.writePort, args=(chosen_shutter, open_shutter))
    main_processes = [spectrometer_init_pros, spectrometer_read_pros, daq_process, shutter_process]
...
def Perform_Test():
    ...
    # Check to see if the output folder named Records exists (as a subfolder of the folder that contains this program), creates this folder if it does not already exist
    Check_Output_Folder()
    ...
    duration = float(record_time.get())*1000.0 + num_background_Spec*float(integration_time.get()) #Recording duration in s

    integration_list_ms = [8, 16, 32, 64, 128, 256, 512, 1024] #integration times for multi-paradigm
    shutter_delay = 4 #ms
    ...
    #initialise number of PM/Photodiode readings based on the time needed per sample (as determined by speed tests) and the duration of the reading
    num_DAC_samples = int(round(duration/DAQ_samplingrate))

    if power_meter.Error == 0:
        num_power_samples = int(round(duration/power_samplingrate))
    else:
        num_power_samples = 0
    #About to start the paradigm - using all of the previously calculated values
    ...
    num_background_Spec = 10       # This is for continuous reading and refers to the last few spectrom reading wich are background and the laser is off

    if (paradigm_mode.get() == 'c'): #Continuous paradigm
        num_Spectrometer_Samples =  int(round(float(DurationOfReading)/float(Integration_Continuous)))  # Number of samples for spectrometer to read.
    else: #Multi Integration
        num_Spectrometer_Samples =  len(Integration_list_MilSec) # Number of samples for spectrometer to read.

    #Takes data from devices to determine the range of appropriate wavelengths to measure
    global wavelengths, min_wave_index, max_wave_index, Spec_Time, Spectrometer_Index
...
'''
    ...
    while True:
        ...
        if (paradigm_mode.get() == 'm'):
            ...
            Continuous_Paradigm(float(Integration_Continuous), num_Spectrometer_Samples, num_DAC_Samples, num_power_samples, num_background_Spec)
        ...
        #PLots the readings for the powermeter -- Won't plot if the PM is not connected
