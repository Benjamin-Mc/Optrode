...
def Continuous_Paradigm(Integration_Continuous, Number_of_Spectrometer_Samples, Number_of_DAC_Samples, number_of_power_samples, Number_of_BakGro_Spec):
    ...
    while (int(Spectrometer_Index[0]) < Number_of_Spectrometer_Samples):
        ...
        if  Spectrometer_is_read.value == 1:
            Spectrometer_is_read.value = 0
            Full_Spec_Records_2D[Spectrometer_Index[0], :] = Current_Spec_Record
...
def Perform_Test():
    ...
    Spec_SamplingRate = Spectrometer_Speed_Test(Number_of_Spec_Tests)
    ...
    while True:
        ...
        global Full_Spec_Records_2D, DAQ_Signal, DAQ_Time, DAQ_Index, DAQ_Index_Total, Ref_Signal, Ref_Time
        Current_Spec_Record = np.zeros(shape=(len(Wavelengths), 1), dtype = float)
        Full_Spec_Records_2D = np.array(np.zeros(shape=(len(Wavelengths), Number_of_Spectrometer_Samples), dtype = float)) #Values in 2D Array
        ...
        Spec_Records_1D = np.ndarray.reshape(Full_Spec_Records_2D, len(Wavelengths)*Number_of_Spectrometer_Samples)
        ...
        plt.figure()
        for i in range(Full_Spec_Records_2D.shape[1]):
            plt.plot(np.asarray(Spec1.readWavelength()[Min_Wave_Index:Max_Wave_Index+1]), Reshaped_Spec_Records[i, :])
