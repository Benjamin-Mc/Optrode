...
def mainplots(intensities, wavelengths, export_pdf, page_heading, background, titles):
    ...
    col_titles = ["Peak Value\n (Sum)", "Peak Value\n (Average)", "Average Intensity across\n all Wavelengths", 
                    "Standard deviation\n (Sum)", "Standard deviation\n (Average)"]
    row_titles = titles
    data = [[], [], [], [], []]
    for i in range(num_samples):
        data[0].append(max(sum_intensities[i]))
        data[1].append(max(avg_intensities[i]))
        data[2].append(sum(avg_intensities[i])/float(len(avg_intensities[i])))
        data[3].append(np.std(sum_intensities[i]))
        data[4].append(np.std(avg_intensities[i]))

    df = pd.DataFrame({col_titles[0]:data[0], col_titles[1]:data[1], col_titles[2]:data[2], col_titles[3]:data[3], col_titles[4]:data[4]})
    df.index = row_titles
    ...
    table = ax.table(cellText=df.values, colLabels=df.columns, rowLabels=row_titles, loc="right", cellLoc="center", bbox=[l, b, w, h])
...
def generate_output(sample_file, background_file, document_title):
    ...
    with PdfPages(r"/home/frederique/PhysicsLabPythonCodes/Optrode/Records/{}.pdf".format(document_title)) as export_pdf:
        ...
        intensities_b, times_b, wavelengths_b = [background_data[4]], [background_data[5]], [background_data[6]]
